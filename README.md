...
현재 상태: **구현 착수 — 3일차 진행 완료 (2026-07-23 기준)**. Phase 1 기반 계약 구축 중이며, 화면 코드(`src/app/`)는 아직 create-next-app 기본 파일뿐입니다(라우트 골격은 9~13일차).
...
| **002** 도메인 타입 47종 | 1팀 | 1/7 | 🔄 진행 중 — `src/types/` 11파일 골격 + E-01~E-08 정의 |
| **006** 시드 PRNG·결정론 유틸 | 2팀 | 3/5 | 🔄 진행 중 — `prng.ts` `derive.ts` `precision.ts` |
...
src/lib/sim/rng/  3파일 — prng.ts(xoshiro128**) / derive.ts(시드 계층) / precision.ts(6자리 정수 비교)
```

**8일차 동결 전 반드시 처리해야 할 것** (3일차 교차 점검에서 발견, `docs/ISSUES.md`)

- **I-31** `World`에 시간 환산 앵커 필드 결손 — 미반영 시 H-24 계약 불성립 → 5팀 015 연쇄 지연 (1팀 5일차)
- **I-32** `Seed` 32비트 → 53비트 완화 — 실효 payload 30비트로 시즌당 충돌 수백 건. 완화 시 I-29도 함께 해소 (1팀 5·7일차)
