...
## 진행 현황 (60일차 · 2026-10-12)

**사이드 내비에서 "준비 중"으로 잠겨 있던 항목 하나가 오늘 열렸습니다.** 구단 목록(`/teams`)은 계획상 아무 Task에도 속하지 않은 채 방치되다 49일차에 정식 배정됐고, 오늘 실제 화면이 생겼습니다. 리그를 고르면 소속 구단이 **팀명순**으로 나열됩니다 — 순위순으로 세우고 싶은 유혹이 있었지만, 그러면 "이 목록은 순위표다"라는 없는 의미가 붙습니다. 같은 이유로 성적·재정 같은 집계 열도 넣지 않았습니다. 명세가 없는 화면에서 지표를 늘리면 "무엇을 기준으로 고른 목록인가"가 임의로 생깁니다.

**화면이 생긴 그날 내비 잠금도 함께 풀었습니다.** 일정표는 이 잠금 해제를 62일차에 몰아서 하도록 잡아 뒀지만, 그러면 오늘 만든 화면이 이틀간 메뉴에서 눌리지 않습니다. 실제로 49일차에 선수 상세 화면이 "앱 내 링크 0건"이라 URL을 직접 쳐야만 닿는 상태였던 전례가 있습니다. 같은 파일의 44일차·50일차 기록도 둘 다 "화면이 생긴 그 일차에 풀었다"였기에 선례를 따랐습니다. 아직 화면이 없는 경기(`/matches`)는 그대로 잠가 뒀습니다.

**전 화면을 6개 화면 크기 × 2개 언어로 훑어 가로 스크롤을 점검했고, 딱 한 곳이 걸렸습니다.** 클럽 상세 화면이 **1024px에서만** 264px 밀려났습니다. 좁은 화면에서는 한 열로 쌓여서, 넓은 화면에서는 여유가 있어서 둘 다 안 보이고 정확히 그 중간 크기에서만 드러나는 종류입니다. 원인은 2열 분할의 각 열이 "내용물보다 좁아질 수 없다"는 기본 규칙에 걸린 것이었습니다.

**발견한 팀은 고치지 않고 사실만 보고했습니다.** 그 파일은 다른 팀 소유입니다. 팀장이 직접 재현해 수치를 확인한 뒤 소유 팀에 넘겼고, 소유 팀이 고쳤습니다(264px → 0). 급한 마음에 "넘치는 부분을 숨기는" 손쉬운 처리를 하면 글자가 잘려 나가므로, 그 방법은 쓰지 말라고 미리 못 박았습니다.

**고치고 나니 다음 문제가 드러났고, 그것도 덮지 않았습니다.** 밀려나던 압력이 사라진 대신 오른쪽 열이 좁아져 팀 이름이 네 글자쯤에서 잘립니다. 추적해 보니 코드 잘못이 아니라 **설계 문서의 공백**이었습니다 — 화면 설계서는 "1024px 이상에서 62:38로 나눈다"고만 적었을 뿐, 그 38%가 가장 좁아질 때 안에 든 카드가 성립하는지는 따지지 않았습니다. 오늘 범위를 더 넓히는 대신 선택지 세 가지를 정리해 이슈로 남겼습니다(I-291).

**관리자 콘솔의 "숨김"이 이제 완전해졌습니다.** 콘솔을 꺼 두면 화면과 기능은 "없는 페이지"로 응답했지만, 정작 가장 앞단인 미들웨어만 "권한 없음"으로 답하고 있었습니다. 차이가 사소해 보여도, 전자는 경로의 존재 자체를 숨기고 후자는 "여기 뭔가 있다"고 알려 줍니다. 오늘 앞단도 같은 방식으로 통일했습니다. 아울러 이 파일을 **어느 팀이 소유하는지가 6일간 공백**이었던 것도 정리했습니다 — 앱 전체의 단일 관문이라 여러 팀이 동시에 손대면 충돌하므로, 소유는 한 팀으로 두되 "판단 로직은 각 팀 파일에 두고 여기서는 불러다 쓴다"는 규칙을 파일 맨 위에 적어 다음 사람이 열자마자 보이게 했습니다.

---

## 진행 현황 (59일차 · 2026-10-09)

**"이미 완료"로 6일간 표시돼 있던 항목이 실은 절반만 구현돼 있었습니다.** 운영 콘솔의 보호 장치는 두 겹입니다 — 1차는 환경 플래그로 콘솔을 통째로 끄는 킬스위치, 2차는 로그인·권한 확인입니다. ROADMAP은 54일차부터 이 항목을 완료로 표기했지만, 실제로 만들어진 것은 2차뿐이었습니다. 1차 플래그는 "변수명을 뭘로 할지"가 미결(W-45)로 남은 채 한 번도 존재한 적이 없었습니다. 오늘 5팀이 그 공백을 메웠고, 표기가 앞서갔던 경위도 ROADMAP에 함께 남겼습니다.

**플래그는 "설정하지 않으면 꺼짐"으로 잡았습니다.** 반대로 했다면 배포 환경에서 값을 깜빡 잊었을 때 관리자 콘솔이 그대로 열립니다. 그래서 정확히 `"true"`일 때만 켜고 미설정·오타는 전부 차단합니다. 부작용으로 **새로 받은 체크아웃이나 CI에서는 콘솔 세 개가 404로 뜨는데, 그게 정상 동작입니다** — 회귀로 오해하기 쉬운 자리라 인계에 명시했습니다.

**"공개 API에 레이트 리밋을 걸었다"는 보고를 확인해 보니 실제 공개 API에는 한 건도 걸려 있지 않았습니다.** 6팀은 분당 300건 제한을 어드민 로그인 엔드포인트에 걸고 이를 공개 버킷으로 보고했지만, 그 라우트는 공개 API가 아니라 인증 창구입니다(6팀 자신의 주석도 목적을 "무차별 대입 방어"로 적고 있었습니다). 진짜 공개 라우트인 라이브 경기 조회 2종은 `grep`으로도 `curl`로도 무방비였습니다. 재수정 후 **305회를 실제로 쏴서 300건 통과 뒤 429가 떨어지는 것을 동작으로 확인**했습니다 — 코드가 아니라 응답으로 본 것이 오늘 검증의 핵심입니다.

**반대로 "안 거는 게 맞다"고 판정한 것도 있습니다.** 헬스체크 엔드포인트는 다른 요구사항이 "항상 200"을 계약으로 걸어 뒀고, 여기에 제한을 걸면 모니터링 자체가 차단됩니다. 6팀이 스스로 보류하고 사유를 밝힌 판단을 그대로 수용하되, 다음에 같은 논의가 반복되지 않도록 근거를 이슈로 남겼습니다(I-285).

**RLS 정책 테스트는 정책을 베껴 쓰지 않고 마이그레이션 SQL을 직접 읽습니다.** 액터 4종 × 오퍼레이션 4종 × 테이블 11개 192건인데, 기대값을 테스트에 하드코딩했다면 정책이 바뀌어도 테스트는 계속 통과했을 것입니다. 파싱 방식이라 정책을 고치면 테스트가 따라 깨집니다. 다만 공개 읽기 34개는 **로컬에 SQL 파일 자체가 없어**(I-263) 대상 밖이며, 이 공백은 그대로 남아 있습니다.

**보안 경고 "0건" 기준은 문자 그대로는 미달했고, 그 사실을 덮지 않았습니다.** 경고 2건이 관측됐는데 추적해 보니 그중 하나는 정의가 로컬 어디에도 없고 원격 DB에만 존재했습니다 — 문서화되지 않은 원격 변경의 흔적입니다. 오늘 작업과 무관한 사전 존재분이라 "신규 유입 0건"으로 통과 판정하되, 실제 해소는 별도 이슈로 이관했습니다(I-286).

---

## 진행 현황 (58일차 · 2026-10-08)

**운영 콘솔 세 번째 화면 `/admin/scheduler`가 열렸고, 감사 기록이 고쳐 쓸 수 없게 됐습니다.** 5팀은 크론이 마지막으로 언제 돌았는지, 성공률과 소요 시간은 어떤지, 중간에 빠진 구간은 없는지를 한 화면에 모았습니다. 6팀은 감사 테이블 두 개(`audit_log`·`common_code_history`)에 UPDATE/DELETE를 막는 마이그레이션을 작성했습니다.

**"RLS로 막았다"는 말은 여기서 정확하지 않습니다.** service_role은 RLS를 우회하기 때문입니다. 그래서 정책만 거는 대신 UPDATE/DELETE **정책을 아예 만들지 않고**(외부 API 경유 요청의 1차 차단선), 그 뒤에 예외를 던지는 트리거를 둬서 역할과 무관하게 거부하도록 했습니다. 12일차 설계 문서가 이미 지적해 둔 것을 오늘 처음 실제로 적용한 것입니다.

**"베팅 감사 테이블"은 만들지 않았습니다.** 작업표 문구만 보면 별도 테이블이 필요해 보이지만, 스키마 설계상 `audit_log` 하나가 배당·정산 액터까지 포괄합니다. 문구를 따라 테이블을 하나 더 만드는 대신 근거를 확인하고 대상을 2개로 확정했습니다.

**크론 화면은 지금 대부분 비어 있고, 그게 맞습니다.** 크론 점등이 아직 금지돼 있어(I-214) 월드가 실제로 한 번도 돌지 않았기 때문입니다. 화면은 조회만 하고 기동 코드는 한 줄도 없습니다. "다음 예정" 시각도 주기를 하드코딩하지 않고 공통코드에서 읽어와, `/admin/config`에서 주기를 바꾸면 함께 움직입니다.

**500이 나던 API 버그를 하나 잡았습니다.** 어드민 로그인 API에 body로 JSON `null`을 보내면 서버가 터졌습니다 — 입력이 잘못된 것이니 400이어야 할 자리였습니다.

**팀장 과실로 같은 팀이 두 번 소환되는 사고가 있었습니다.** 종료 통보 직후 작업 트리가 비어 있는 것을 "작업 없이 끝났다"로 읽고 재소환했는데, 실제로는 양 팀 모두 작업 중이었습니다. 5팀은 같은 파일을 두 인스턴스가 동시에 쓰는 실제 경합까지 갔습니다. 다만 **양쪽 모두 이상을 먼저 자진 보고했고**, 금지된 git 복구 조작 없이 정본을 지정해 정리해 산출물 손실은 0건입니다. 재소환 전 프로세스 확인을 필수 절차로 올렸습니다(I-284).

최종 게이트는 **1860건 통과**, 회귀 0건, 재수정 0회. 신규 이슈는 I-283(감사 조회 권한과 화면의 접근 경로 불일치)·I-284(중복 기동) 2건이며 둘 다 비차단입니다. **I-283은 원격 마이그레이션 적용 전에 반드시 먼저 처리해야 합니다** — 적용 순간 어드민 화면 두 곳이 에러 없이 조용히 빈 목록을 보이게 됩니다.

---

<details>
<summary>57일차 (2026-10-07)</summary>
