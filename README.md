...
**마감 커밋 직후, "선수 카드는 어떻게 보느냐"는 질문 하나가 빈틈을 드러냈습니다.** 오늘 만든 선수 상세는 정상 동작했지만 **앱 안에서 그 화면으로 가는 링크가 한 곳도 없었습니다** — 전 화면 grep 결과 `players/` 링크 0건, 사이드 내비의 "선수"는 36일차 이래 비활성. URL을 직접 쳐야만 볼 수 있었던 것입니다. `/stats`·`/awards`의 선수명 3곳을 링크로 바꾸고, **`/[lang]/players` 인덱스를 신설**해 내비를 열었습니다(I-223 2차 부분 해소, 잔여 3종). `DataSource`에 전 선수를 나열하는 계약이 없어 **리그→구단→스쿼드로 좁히는 형태**가 됐습니다. 교훈은 단순합니다 — **"화면을 만들었다"와 "사용자가 그 화면에 갈 수 있다"는 서로 다른 판정**이고, 오늘 검증은 앞의 것만 했습니다. 작업 중 stale `.next/types/`가 신규 라우트 typecheck를 오탐시키는 것도 발견했습니다(I-254 — 오늘 해소한 I-242의 사각지대). 신규 이슈 2건 추가(I-253·I-254).
