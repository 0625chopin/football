...
현재 상태: **구현 착수 — 34일차 진행 완료 (2026-09-04 기준)**. 8일차에 도메인 타입 47종이 **동결(H-01)** 됐고, 지금은 그 위에서 6팀이 병렬로 계약·엔진·상수·물리 스키마를 쌓는 단계입니다. **34일차부터 화면이 실제로 보입니다** — `/[lang]` 홈에 3리그 라이브 경기 카드가, `/[lang]/sample`에 컴포넌트 21종이 렌더됩니다(그 전까지는 `params`를 JSON으로 찍는 자리표시자였습니다). DB에는 39테이블 + 인덱스 20개 + RLS 39/39가 실제로 적용돼 있습니다. 15일차에 **머지 게이트(`npm run gate`)** 가 가동돼 `tsc → lint → test:coverage`가 병합 전 강제되고, **20일차에 이 게이트가 GitHub Actions CI(`.github/workflows/ci.yml`)에도 연결**됐습니다. **24일차에 `next typegen`이 앞단에 추가돼 4단이 됐습니다** — 로컬에만 남아 있던 Next 생성 타입 때문에 로컬은 통과하고 CI만 실패하던 위양성을 없앤 것입니다(I-138).
