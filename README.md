...
현재 상태: **구현 착수 — 20일차 진행 완료 (2026-08-17 기준)**. 8일차에 도메인 타입 47종이 **동결(H-01)** 됐고, 지금은 그 위에서 6팀이 병렬로 계약·엔진·상수·물리 스키마를 쌓는 단계입니다. **화면은 라우트 골격 + 전역 레이아웃 + 로케일 라우팅까지 완성됐고(Task 005 종료), DB에는 39테이블 + 인덱스 20개 + RLS 39/39가 실제로 적용됐습니다** — 화면 본문은 전부 자리표시자이며 5팀이 34일차 이후 채웁니다. 15일차에 **3단 머지 게이트(`npm run gate`)** 가 가동돼 `tsc → lint → test:coverage`가 병합 전 강제되고, **20일차에 이 게이트가 GitHub Actions CI(`.github/workflows/ci.yml`)에도 연결**됐습니다.
...
| **011** i18n 기반 | 4팀 | 🔄 로케일 라우팅 + 카탈로그 8네임스페이스 + 키 타입 파생 + 서버·클라 겸용 `t()` / `TranslationProvider` + **열거형 카탈로그 골격 66키**(19일차, 값은 3팀이 23일차 이후) + **날짜·숫자 포맷터 단일 소스**(20일차, `src/**`에서 `Intl.*` 직접 호출 0건 실증) — 로케일 스위처·Provider 실배선은 22일차 |
...
| **024** 능력치 보정 체인 | 2팀 | 🔄 계수 9종 골격·클램프 + 컨디션·피로·캐미 실공식 + 포지션 숙련도 BFS 페널티 + **날씨·감독 6×6 상성**(20일차, `tactics.ts` 분리 확정, 리터럴 0건) — 실제 계수 값은 36일차 시드 이후 반영(I-118). 잔여: 라인업 자동 선정·카드 정지·I-03 |
| **029** 포인트 경제 | 3팀 | 🔄 **포인트 원장**(20일차) — `postPointTransaction` 단일 진입점 + `deriveBalance` 항등식(NFR-QA-005), DC-08 정수 고정. 잔고 직접 변경 함수 부재로 수락 기준 구조적 강제. 영속화 미배선 |
| **034a** Supabase 어댑터 | 6팀 | 🔄 **1/3 — `getStandings`·`getFixturesByRound`**(20일차). `@supabase/*` 미설치를 클라이언트 주입 인터페이스(duck-typing)로 해소, `mapper.ts` 재사용, `factory.ts` 미등록(3/3 후) |
| **044** CI 게이트 | 1팀 | 🔄 **3단 게이트 CI 실행**(20일차) — push·PR(master)에서 `npm run gate` 단일 호출 + `check:literals` advisory. ⚠️ **러너 첫 실행 미확인**. 잔여: 시드 스냅샷·번역 키 검사·시크릿 스캔 |
