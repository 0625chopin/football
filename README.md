...
현재 상태: **구현 착수 — 13일차 진행 완료 (2026-08-06 기준)**. 8일차에 도메인 타입 47종이 **동결(H-01)** 됐고, 지금은 그 위에서 6팀이 병렬로 계약·엔진·상수·물리 스키마를 쌓는 단계입니다. **화면은 라우트 골격 + 전역 레이아웃 + 4상태 파일까지 완성됐고(Task 005 종료), DB에는 핵심 32테이블이 실제로 생성됐습니다** — 화면 본문은 전부 자리표시자이며 5팀이 34일차 이후 채웁니다.
...
> ⚠️ **WSL 마운트 경로(`/mnt/...`) 환경 주의 (I-62)** — Turbopack은 파일 쓰기 실패로 구동되지 않습니다. **`npm run dev`는 `next dev --webpack`으로 고정돼 있으니 그대로 쓰면 됩니다**(12일차 조치). `npx next dev`(플래그 없음)로 직접 띄우지 마세요 — EPERM으로 죽고 루트에 스트레이 디렉터리를 만듭니다(I-87).
>
...
| **003** 공통코드 카탈로그 | 3팀 | ✅ **완료** (12일차) — 36그룹·로더·폴백·발효정책·스냅샷 해시. **H-05 인계** |
...
| **023** 매치 틱 엔진 | 2팀 | 🔄 순회·이벤트 23종·스탯 폴드·교체·**승부차기**(13일차) — GK 폴백은 14일차 |
| **007** Mock 월드 팩토리 | 3팀 | 🔄 **국적 기반 이름 생성기**(13일차, D-16/D-17) — 엠블럼 SVG는 14일차 |
| **008** 테스트 하네스 | 1팀 | 🔄 `test:watch`/`test:coverage`·스위트 골격 6종(13일차) — 커버리지 임계는 14일차 |
| **032** DB 마이그레이션 | 6팀 | 🔄 **핵심 32테이블 실제 생성**(13일차) — 공통코드·운영 7테이블은 14일차 |
...
src/lib/sim/match/ tick.ts — 90(+30)틱 순회, 추가시간(전반 0~5 / 후반 1~8)  ← 9일차
                   ...
                   penalty.ts — 승부차기 5+서든데스, PK골 통산 미포함(D-19) ← 13일차 신규
src/lib/naming/    generate.ts — 국적 기반 결정론 이름 생성(D-17)          ← 13일차 신규
...
src/lib/config/    catalog.ts — 공통코드 그룹 36종                        ← 9일차
                   loader.ts — 그룹코드·반환타입 컴파일타임 강제 로더       ← 10일차
                   fallback.ts(36그룹 안전 기본값) / policy.ts(발효 3종)   ← 11일차
                   snapshot.ts — 상수 스냅샷 SHA-256·중복 제거            ← 12일차 신규
...
supabase/          migrations/*.sql — 핵심 32테이블(E-01~E-32) 실제 적용   ← 13일차 신규
docs/handoff/      H-10-enum-display-names.md — 열거형 ko/en 표시명 70건  ← 13일차 신규
docs/db/           schema-design.md — 47엔티티 매핑·관계·인덱스·RLS 초안
                                      + §8 타입 필드 대응표(불일치 0건)    ← 12일차
vitest.config.ts   @/* 별칭 해석 + *.type-test.ts는 typecheck 모드로 검증  ← 12일차 신규

npm run test → 24 files / 291 tests 통과 · Type Errors 0 · npx tsc --noEmit 0
```

> ⚠️ `npm run lint`는 리포지토리 루트의 스트레이 빌드 캐시 디렉터리 때문에 무관한 오류를 대량 출력합니다(I-87). 실제 소스 판정은 `npx eslint src vitest.config.ts`(현재 **0건**)로 하세요.
