...
현재 상태: **구현 착수 — 18일차 진행 완료 (2026-08-13 기준)**. 8일차에 도메인 타입 47종이 **동결(H-01)** 됐고, 지금은 그 위에서 6팀이 병렬로 계약·엔진·상수·물리 스키마를 쌓는 단계입니다. **화면은 라우트 골격 + 전역 레이아웃 + 로케일 라우팅까지 완성됐고(Task 005 종료), DB에는 39테이블 + 인덱스 20개 + RLS 39/39가 실제로 적용됐습니다** — 화면 본문은 전부 자리표시자이며 5팀이 34일차 이후 채웁니다. 15일차에 **3단 머지 게이트(`npm run gate`)** 가 가동돼 `tsc → lint → test:coverage`가 병합 전 강제됩니다.
...
| **007** Mock 월드 팩토리 | 3팀 | 🔄 월드 팩토리·진행 상태·4상태 픽스처 + **`MockDataSource` 56메서드 전량**(18일차) — I-106 완전 해소. 배럴 등록만 19일차(I-113) |
...
| **011** i18n 기반 | 4팀 | 🔄 로케일 라우팅 + 카탈로그 8네임스페이스 + 키 타입 파생 + **서버·클라 겸용 `t()` / `TranslationProvider`**(18일차) — 실배선은 22일차 |
| **010** 코드 규약·정적 가드레일 | 1팀 | 🔄 sim 결정론 lint 가드 + import 가드 + **D-18 하드코딩 JSX 문자열 룰·숫자 리터럴 검사 스크립트**(18일차) — 룰 적용 시 기존 placeholder에서 112건 warn 실증 |
| **024** 능력치 보정 체인 | 2팀 | 🔄 계수 9종 골격·클램프 + **컨디션·피로·캐미 실공식 3종**(18일차, 경계값 전건 통과) — 잔여 5종은 19~20일차 |
...
npm run test → 542 tests 통과(44파일) · Type Errors 0                ← 18일차 기준
