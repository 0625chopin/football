...
| **032** DB 마이그레이션 | 6팀 | 🔄 39테이블 + RLS 39/39 + **제약(CHECK 142건·트리거 2종)·FK 인덱스 65건**(16일차) — advisors 성능 **2종**만 18일차까지 이월 |
| **011** i18n 기반 | 4팀 | 🔄 로케일 라우팅 확정 + **메시지 카탈로그 8네임스페이스 × ko/en**(16일차) — ko 타입으로 en 키 누락·고아 키를 컴파일타임 차단 |
| **010** 코드 규약·정적 가드레일 | 1팀 | 🔄 **`src/lib/sim/**` 결정론 lint 가드**(16일차) — `no-restricted-properties`로 `Math.random`/`Date.now` 차단(I-105) |
