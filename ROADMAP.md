...
- **구현 사항**
  ...
  - [~] 테이블 시맨틱 마크업(`<caption>`, `scope`) 및 모바일 가로 스크롤 컨테이너 — 순위표는 충족. **일정/결과 목록(C2)은 `MatchCard` row 재사용을 위해 `<ul>`로 마크업해 NFR-A11Y-005 편차(I-210)** — C2가 표인지 카드 리스트인지 판단 선행
...
- **구현 사항**
  ...
  - [ ] 공통코드 폴백 발생 시 WARN + 콘솔 카운터
    - ⚠️ WARN 자체는 `config/fallback.ts`가 이미 출력하나, **41일차 현재 그 경고가 dev 로그에 45건씩 쏟아지는 상태**다 — 6팀이 `common_code`에 38그룹/155코드를 적재했는데도 `setGlobalDefaultSource()` 호출처가 0건이라 앱이 값을 조회하지 않는다(**I-206**, 1팀 배정 필요). 폴백 경고가 상시 노이즈가 되면 이 항목의 카운터도 의미를 잃는다
