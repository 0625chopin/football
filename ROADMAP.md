...
| # | 역할 | 주 담당 요구사항 | 매핑 에이전트 |
|---|---|---|---|
| 1 | 시장 조사 담당 | 사업 트랙 ✅ 완료 | 901 |
| 2 | 경쟁사 분석 담당 | 사업 트랙 ✅ 완료 | 902 |
| 3 | 예산안 담당 | 사업 트랙 ✅ 완료 | 903 |
| 4 | 아키텍트 / 타입 설계 | E-01~E-47, FR-UI-023, NFR-MT-001·002·009, DC-01·10 | `nextjs-app-developer` |
| 5 | 시뮬레이션 엔진 | FR-LG, FR-MT, NFR-DT, NFR-PF-001~003 | (신규) |
| 6 | 데이터 / 밸런싱 | FR-PL, FR-TM, FR-TR, FR-EC, FR-YT, FR-AW, FR-AD-011~016, NFR-CFG | (신규) |
| 7 | 배팅 / 배당 도메인 | FR-BT, NFR-DT-006, NFR-SEC-004~006 | (신규) |
| 8 | UI / 컴포넌트 (**i18n 주담당**) | FR-UI-000~026, FR-ST-004, NFR-RS, NFR-A11Y, D-18 | `ui-markup-specialist` |
| 9 | DB / Supabase | E-01~E-47 물리 스키마, FR-AD-017~022, NFR-CR, NFR-SEC, NFR-SC | `nextjs-supabase-expert` |
| 10 | QA / 테스트 | NFR-QA 전량, NFR-MT-005, KPI 검증 | `code-reviewer` + Playwright MCP |

### Phase별 업무 분담 매트릭스 (◎ 주담당 / ○ 지원 / – 미참여)

| Phase | 1 시장 | 2 경쟁 | 3 예산 | 4 아키텍트 | 5 엔진 | 6 데이터 | 7 배팅 | 8 UI | 9 DB | 10 QA |
|---|---|---|---|---|---|---|---|---|---|---|
| **Phase 1 골격** | – | – | ○ | ◎ 001·002·004·005·010·011 | ○ 006 | ◎ 003·007·011 | ○ 003 | ◎ 011 / ○ 005 | ◎ 009 | ◎ 008·010 |
| **Phase 2 UI** | – | – | – | ○ 012·014 | – | ○ 021 | ○ 020 | ◎ 012~022 | – | ◎ 022 |
| **Phase 3 기능** | – | – | ○ 033 | ○ 034 | ◎ 023~028 | ◎ 029~031 | ◎ 035 | ○ 035 | ◎ 032~034 | ◎ 036 |
| **Phase 4 고급** | – | – | ○ 044 | ○ 045 | ○ 042 | ○ 043 | ◎ 039~041 | ○ 039 | ◎ 037·038·042 | ◎ 041·044 |
...
- **구현 사항**
  ...
  - [ ] 단일 월드 전제(D-15)를 타입·스키마 설계 원칙으로 명문화 (전 쿼리 `world_id` 필터 미도입, 확장 여지 한 줄만 유지)
  - [ ] 절차적 생성 원칙(D-16) 명문화 — 외부 축구 데이터 소스·실명 자산 도입 금지를 코드 리뷰 체크리스트에 추가. **D-17의 실명 블랙리스트 필터**를 Task 007 요건으로 전달
  - [ ] D-19(승부차기 득점 미포함, **연장전 득점은 포함**)·D-20(감독 독립 엔티티 + 최소 이적)을 Task 023·028·030의 구현 전제로 명시
  - [ ] **D-21~D-26을 해당 Task의 구현 전제로 배분** — D-21 재임대 금지 → Task 030 / D-22 대체 GK 선정 우선순위 → Task 023·024 / D-23 감독 공석 0라운드·즉시 대행 → Task 030 / D-24 컵 시딩 폴백 → Task 027 / D-25 확률 밴드는 유지하고 엔진 상수를 조정 → Task 008·031 / D-26 공통코드 시드값을 `default_value`로 확정 → Task 003·031
...
- **담당**: 팀원 4 (아키텍트) / 리뷰: 팀원 5·6·9
- **근거**: E-01~E-47, D-15, DC-01, DC-10, NFR-MT-002, NFR-MT-009
- **구현 사항**
  - [ ] `src/types/` 하위를 도메인별로 분할 (`world.ts`, `person.ts`, `match.ts`, `stat.ts`, `economy.ts`, `betting.ts`, `config.ts`, `ops.ts`, `index.ts`)
  - [ ] E-01~E-32 (1차 범위) 전량 정의 + E-33~E-40(배팅·사용자)은 2차 대비 타입만 선정의. **E-01 World는 단일 레코드 전제**(D-15)
  ...
  - [ ] 34속성(FR-PL-002) 타입, 시드 계층 타입(world/season/match), 상수 스냅샷 타입(E-44)
...
- **담당**: 팀원 5 (시뮬레이션 엔진) / 리뷰: 팀원 10
- **근거**: FR-MT-015, FR-AD-003, NFR-DT-001~005·008
- **구현 사항**
  - [ ] `src/lib/sim/rng/` 에 xoshiro128\*\* 또는 mulberry32 구현 (순수 함수, 상태 명시적 전달)
  - [ ] 시드 계층 파생 함수 — `world_seed → season_seed → match_seed → event_seed`, 배당 프리시뮬은 **독립 네임스페이스**(NFR-DT-006)
  - [ ] 고정 정밀도(소수 6자리 반올림) 확률 비교 헬퍼 (NFR-DT-005)
  - [ ] 안정 정렬 헬퍼 — 명시적 tiebreak 키 필수 (NFR-DT-008)
  - [ ] 상태 해시(정렬 직렬화 + SHA-256) 유틸 (NFR-DT-003)
- **수락 기준**: 동일 시드 100만 회 추출 결과가 재실행 시 바이트 단위 동일. 단일 경기 재현 ≤ 100ms를 가능케 하는 시드 파생 구조.
- **테스트**: Vitest — 시드 재현성, 분포 균등성, 입력 순서 셔플 불변성.

### Task 007: 시드 기반 결정론적 Mock 팩토리를 구축한다

- **담당**: 팀원 6 (데이터/밸런싱) / 의존: Task 002·003·006
...
- **담당**: 팀원 8 (UI/컴포넌트, 주담당) + 팀원 4 (키 규약·타입 안전성) + 팀원 6 (열거형 표시명 카탈로그)
...
- **담당**: 팀원 8 (UI/컴포넌트) / 지원: 팀원 7(배당 표기 규약)
...
- **담당**: 팀원 8 (UI/컴포넌트) / 지원: 팀원 6(공통코드 스키마)·팀원 9(크론 지표)
...
- **담당**: 팀원 6 (데이터/밸런싱) / 리뷰: 팀원 10(회계 항등식)
...
- **담당**: 팀원 6 (데이터/밸런싱) / 지원: 팀원 9(시드 마이그레이션)
...
- **담당**: 팀원 9 (DB/Supabase) + 팀원 5 (엔진) / 입력: 팀원 3(요금제·실행 빈도 예산)
...
- **담당**: 팀원 10 (QA/테스트) / 지원: 팀원 3(예산·요금제 입력)
