...
- **구현 사항**
  ...
  - [x] **소급(4팀 다음 등판, D-35 / I-239) — `/sponsors` 계약 상세 표에 체결 구단주 열 추가** — **60일차 완료**. 3팀 H-27(48일차, I-231 해소) 선행을 확인하고 착수했다. `SponsorContract.signedByOwnerId`는 조회 전용이라 그 ID로 되짚는 메서드가 없어 **`contract.teamId → getClubOwner` 조인으로 대체**했고, 그 조인이 곧 체결 당사자와 일치하는 근거(`world.ts`의 `generateSponsorContractsForTeam`이 계약 생성 시점의 구단주를 그대로 `signedByOwnerId`에 기록하고 이후 구단주 교체 로직이 없음)를 파일 주석에 남겼다. 이름은 고유명사라 **변수 주입**(D-17, 번역 비대상), i18n은 `sponsor.contracts.ownerHeader`/`ownerUnknown` 2키만 확장(ko·en). **팀장 실측: ko「체결 구단주」·en「Signing owner」 열이 실값 렌더**(Federico Greco·Kwame Owusu·Ulises Garcia) → **D-33 판정 기준("계약 0건이면 빈 상태 확인으로 끝나 위반") 충족**. 46일차 주석의 "항상 계약 상세 없음" 문단은 48일차에 이미 사실과 달라져 있어 현재 상태 기록으로 고쳐 썼다
...
- **구현 사항**
  ...
  - [~] `/admin/config` — 그룹별 상수 목록(현재값·기본값·설명·영향 FR), 편집 폼, 범위 검증 인라인 에러, 발효 시점 지정, 변경 이력 diff (사전 설계: `docs/wireframe/10-어드민공통코드-폼스펙.md`, 13일차 5팀). **56일차 H1~H3 완료** — `src/app/[lang]/admin/config/page.tsx`(실경로 주의)에 H1 그룹 목록(검색 필터·코드 수·영향 FR·발효 정책 배지) · H2 코드 테이블(현재값/기본값/단위/활성, JSON 축약) · H3 편집 폼(사유 필수, 저장 서버 액션). 신규 `ConfigGroupNav`·`ConfigCodeTable`·`ConfigEditForm` + `actions.ts`·`config-override-store.ts`, i18n `admin.config.*` ko·en. 위젯은 그룹 `valueType`별 분기하되 `CUP_PARAM.BYE_COUNT`류 **스칼라 래핑 JSON**(`{"value":N}`)은 폼스펙 §4.2대로 파싱 후 shape로 판별해 숫자 입력으로 내리고 저장 시 재포장한다. 저장 검증은 3팀 `validateCommonCodeValue`를 **재사용**(NFR-CFG-004), 액션 첫 줄 `assertAdminSession()`. ⚠️ **그룹 수는 37이 아니라 실측 38**(14일차 `NATIONALITY_WEIGHT` · 31일차 `MANAGER_STYLE_XG` 추가) — 화면이 `groups.length`를 그대로 쓰므로 숫자를 다시 박지 말 것(**I-280**). **57일차 잔여 3건 전량 완료** — ① 범위 검증 인라인 에러: `ConfigEditForm`이 3팀 `getNumericRange`로 숫자 위젯 허용 범위를 실시간 판정(평상시 힌트 → 위반 시 같은 자리를 `role="alert"` 인라인 에러로 교체, `aria-invalid`/`aria-describedby` 연결). 순수 JSON 에디터는 범위 메타데이터가 없어 대상 밖. ② 발효 시점 지정: `actions.ts`가 `applyPolicy==='NEXT_SEASON'` 그룹에 대해 `World.currentSeasonNumber + 1`을 `effectiveFromSeason`으로 계산해 오버레이에 기록(`IMMEDIATE`/`NEXT_MARKET`은 `null` 유지). H3-p 배지는 여전히 표시 전용이며 실제 시즌 번호만 채운다. ③ H4 변경 이력 diff: 신규 `config-history-store.ts`(append-only 오버레이) + `ConfigHistoryDiff`, `page.tsx`가 `getCommonCodeHistory()`(기저)와 로컬 오버레이를 `mergeConfigHistory()`로 병합해 최신순 전달. **`audit-log-store.ts`를 재사용하지 않는다** — 코드값 이력은 별도 엔티티(E-43) 소관. **I-281 해소** — 빈 입력이 `canSave` 게이트에서 차단된다. ⚠️ 단 **서버 액션은 이 판정을 재현하지 못한다**(와이어 타입 `raw: number`가 "빈 입력"과 "명시적 0"을 구분 불가) — 범위 있는 코드는 서버 `validateCommonCodeValue`가 계속 거부하고 범위 없는 코드의 `0`은 정당한 입력이라 **UI 계층 차단으로 종결**. 팀장 검증에서 diff의 이전값↔신규값이 시각 표현 전용이던 것을 `sr-only` 라벨 병기로 재수정(1회). 잔여 이슈: `changedBy` 자리표시자(**I-282**, I-276 동계열), 4그룹 코드 0건(**I-279**), 오버레이 휘발성(**I-273** 확대 — 이제 스토어 3종). 신규 컴포넌트 누적 **11종**의 `/sample` 등록은 4팀 소유라 인계
  - [x] `/admin/scheduler` — 마지막 실행 시각, 성공/실패 이력, 밀린 라운드 수, 중단 구간(`cron_gap`), 잠금 상태 — **58일차 J1~J4 완료**. `src/app/[lang]/admin/scheduler/page.tsx`(실경로 주의)가 11일차 Task 005의 빈 골격을 대체하며, 와이어프레임 `08-어드민-공통코드-스케줄러.md` Part B 기준으로 신규 4종 — J1 `CronStatusSummary`(마지막 실행 경과·잠금 보유·연속 실패·"다음 예정" 추정 배지) · J2 `CronRunTable`(전체/실패/catch-up 필터 + 에러 아코디언) · J3 `CronMetricCards`(성공률·평균 소요·최대 지연, 표본 수 명시) · J4 `CronGapList`(0건이면 섹션 미렌더) + 순수 헬퍼 `scheduler-elapsed.ts`(단위 테스트 7건), i18n `admin.scheduler.*` ko·en(양쪽 175키 완전 일치). **"다음 예정"은 하드코딩이 아니라 공통코드 `CRON_PARAM.INTERVAL_MIN` 조회**라 `/admin/config`에서 주기를 바꾸면 함께 반영된다. **I-214(크론 점등 금지, 차단성) 준수** — `DataSource` 조회 4종(`getLatestCronRun`/`getCronRuns`/`getCronRunMetrics`/`getCronGaps`)만 호출하고 쓰기·기동 코드 0건. `MockDataSource`가 아직 전부 빈 값/0을 반환해 화면은 현재 대부분 Empty 상태이며 이는 "월드가 크론을 한 번도 돌리지 않았다"는 정직한 초기 상태다. **폴링 미도입** — 와이어프레임 **W-48**(폴링 주기)이 판정 대기라 수동 새로고침(서버 컴포넌트 재조회)으로 갈음. 팀장 판정으로 **등재 불요** 처리한 3건: J1 잠금이 `CronRun.lockAcquired` 스냅숏이라 현재 보유를 정확히 표현 못 하는 것(실시간 락 엔티티 부재 + 크론 미점등 상태라 설계상 수용), I-5 중단구간→Fixture 스크롤(이 화면에 Fixture 목록 없음, 스코프 아웃), 새로고침 버튼 부재(W-48 동일 건). 신규 컴포넌트 누적 **15종**의 `/sample` 등록은 4팀 소유라 인계
  - [x] 1차는 비공개 경로 + 환경 플래그로 보호 (NFR-SEC-007) — 54일차 `src/proxy.ts` `/admin/**` 가드 + `actions.ts` 액션별 `assertAdminSession()` 재검증(I-270), **55일차 실측으로 양방향 확인**(무인증·비ADMIN 차단 / ADMIN 통과). ⚠️ **54~58일차의 이 `[x]` 표기는 인증(2차)만 커버한 과대 표기였다** — NFR-SEC-007이 요구하는 **1차 "환경 플래그"는 W-45가 미결로 남긴 채 한 번도 구현된 적이 없었고, 59일차에 비로소 구현됐다**. 신규 `console-flag.ts`가 `ADMIN_CONSOLE_ENABLED`를 **정확히 문자열 `"true"`일 때만** 활성으로 판정하고 미설정·오타·`"1"`은 전부 비활성으로 **fail-closed**한다(`isAuthorizedAdminToken` 선례). 콘솔 3종 `page.tsx`는 `notFound()`로 "비공개 경로"(404 위장)를, 소유 Server Action 5개는 `assertAdminConsoleEnabled()`로 **재검증**한다 — 페이지 검사가 그 안의 Server Action에 이어지지 않는다는 Next.js 16 경고가 이 플래그에도 동일 적용되기 때문. 이 플래그는 인증을 대체하지 않고 앞단에 얹는 **킬스위치**로, 코드 배포 없이 값만 바꿔 세 콘솔을 통째로 숨긴다. **W-45 해소**. ⚠️ 값은 `.env.local`(gitignore)에만 있으므로 **새 체크아웃·CI·배포 환경에서 콘솔 3종이 404인 것이 정상 동작**이다(회귀로 오진 금지). ~~잔여: `proxy.ts`가 플래그를 검사하지 않아 미인증 요청은 여전히 403이지 404가 아니다~~ → **60일차 4팀이 I-272(소유 판정)와 한 패스로 해소(I-287 종결)**. `proxy.ts`가 `isAdminConsoleEnabled()`를 **인증 검사보다 먼저** 호출해 비활성 시 404를 반환하므로, 이제 페이지·Server Action·미들웨어 3계층이 모두 404로 통일됐다. `proxy.test.ts` 3케이스 추가(미인증+플래그off→404 / **유효 관리자 쿠키가 있어도 플래그off→404**로 1차가 2차보다 먼저임을 단언 / `"1"` 오타값 fail-closed). 팀장 실측: 플래그 ON·미인증 `/ko/admin` → **403 유지(회귀 없음)**
  - [x] 위험 조작(리셋·강제 정산)은 2단계 확인 + 사유 입력 필수 — **55일차 리셋 완료**(브라우저 실측: 2단계 진입 후에도 사유·확인문구 미입력 시 「리셋 확정」 disabled, 서버 액션도 동일 함수로 재검증해 클라이언트 우회 불가). **59일차 Playwright 회귀 재확인**(사유+확인문구 미입력 시 비활성 → 입력 시 활성). **"강제 정산"은 이 Task의 대상이 아니다** — 정산 기능 자체가 **Task 040(71일차) 미착수**라 오늘 시점에 보호할 조작이 존재하지 않는다(누락이 아니라 스코프 판정). Task 040 구현 시 같은 `DangerConfirmDialog` 패턴을 적용한다
...
- **구현 사항**
  - [x] 6개 브레이크포인트 × 2로케일에서 전 화면 가로 스크롤 0 검증 — **60일차 완료**. 20화면 × 2로케일 × 320/375/768/1024/1440/1920. **제외 2건은 회귀가 아니다** — admin 3종은 인증 게이트 뒤라 무인증 스윕 대상 밖, `/matches` 인덱스는 아직 미존재(Task 047, 5팀 61일차). 320·375·768·1440·1920은 전부 clean이었고 **1024px에서 `/teams/[teamId]` 단독 오버플로 264px**을 찾아냈다(**I-290**) — `lg:grid-cols-[3fr_2fr]`의 `fr` 트랙이 기본 최소 크기 `auto`(min-content)라 아이템에 `min-w-0`이 없으면 컨테이너 아래로 줄지 못하는 것이 원인(컨테이너 737px vs 트랙 합계 1025px). 4팀은 **5팀 소유 파일이라 사실만 보고**했고, 팀장이 재현한 뒤 5팀이 `min-w-0`으로 해소(264→0, ko·en 확인). ⚠️ 그 대가로 절삭이 `truncate` 스팬으로 이동해 1024px 우측 레일(271px)에서 팀명이 40px까지 잘리는 **I-291이 신규 OPEN**(근본은 코드가 아니라 와이어프레임 3-2절의 스펙 공백). 파생 규칙 — 커스텀 `fr` 그리드는 `grid-cols-[minmax(0,3fr)_minmax(0,2fr)]`로 하한을 명시할 것(홈 `page.tsx:190`이 이미 올바른 선례이고, `players/[playerId]:174`·`admin/config:116`·`matches/[matchId]:260`은 같은 무방비 패턴이나 현재 오버플로 0으로 **잠복**)
...
- **담당**: 5팀 화면·배팅UX팀 / 리뷰: 4팀 UI기반·i18n팀 / 지원: 4팀(내비 `pending` 제거 — `layout.tsx`는 4팀 단독 소유. ~~Task 048 소급분에서 처리~~ → **60일차 당일 처리 완료**, 44·50일차 선례에 맞춤)
...
- **구현 사항**
  - [x] `/[lang]/teams` 구단 목록 — **60일차 완료**. 리그 선택 GET 폼(`?league=`), `getStandings({ leagueId })`로 소속 팀 id → `getTeamsByIds`로 카드(팀명·티어·순위표/스쿼드 링크). **정렬은 팀명순**(`localeCompare` — 순위 순서로 세우면 없는 의미가 붙는다, `players/page.tsx` 49일차 선례). **전용 "리그별 팀 목록" 계약이 없어 `Standing`을 경유**하는 것은 의도된 우회로다. 상호작용이 폼 제출 하나뿐이라 클라이언트 컴포넌트를 만들지 않았다(`/stats`·`/players` 패턴 동일). 그리드는 `grid-cols-1 md:grid-cols-2 lg:grid-cols-3` — `sm:`을 전환점으로 쓰지 않는다(I-184)
  - [x] 세그먼트 4상태 3종(`loading`/`error`/`not-found`) — `RouteLoading`/`RouteError`/`RouteNotFound` 얇은 래퍼(36일차 규약, 개별 마크업 금지). **60일차 완료**
  - [x] i18n `team.list.*` ko·en 신설(`messages/{ko,en}/team.ts`는 5팀 소유) — 하드코딩 0, 팀명·리그명은 변수 주입(D-17). **60일차 완료**, 7키(`description`/`leagueLabel`/`apply`/`tierFormat`/`squadLinkLabel`/`standingsLinkLabel`/`emptyLeagues`) 양 로케일 대칭
  - [x] 320px 가로 스크롤 0 + ko/en 실렌더 실측 후 **4팀에 내비 `pending` 제거를 통지(H-28)** — `src/app/[lang]/layout.tsx`는 4팀 단독 소유이므로 이 팀이 직접 편집하지 않는다. **60일차 완료** — 통지 경로 대신 **팀장이 4팀에 직접 지시해 당일 제거**했다. H-28은 "62일차 일괄"로 잡혀 있었으나 같은 파일 주석의 **44일차(leagues)·50일차(playoffs) 선례가 둘 다 "인덱스가 생긴 그 일차에 해제"** 이고, 미루면 오늘 만든 화면이 이틀간 내비에서 잠긴다(49일차 "앱 내 링크 0건" 사고와 동계열). `matches`는 여전히 404라 그대로 뒀다 → **`pending` 잔여 1건, I-223 종결 조건은 61일차(Task 047)에 충족된다**
- **수락 기준**: 3개 리그 전부 구단 목록이 **실값으로** 렌더되고 각 카드에서 클럽 상세(`teams/[teamId]`)로 이동 가능하다. **스코프 상한** — 성적·재정·스쿼드 규모 등 집계 열은 넣지 않는다(명세 없는 화면에서 지표를 늘리면 "무엇을 기준으로 고른 목록인가"가 임의로 붙는다). → **60일차 충족**(팀장 실측: ko·en 24개 카드 팀명순 실값 렌더, `teams/[teamId]` 링크 이동 정상, 320·1024px 가로 스크롤 0, **집계 열 0개**로 상한 준수).
...
- **담당**: 6팀 DB·인프라팀 / 리뷰: 1팀 코어·품질팀
- **일정**: 54일차 ~ **59일차** (2026-10-02 ~ 2026-10-09) / 추정 3.5인일 / 담당 6팀 DB·인프라팀 — *58일차 정정: `docs/team-schedule/06-DB인프라팀.md`가 58·59일차 행을 갖고 있어 종료일을 맞췄다(일정 단일 소스는 team-schedule)*
- **근거**: NFR-SEC-003·004·008·010·012, DC-05, 02문서 액터 권한 매트릭스
- **구현 사항**
  - [ ] 액터 6종(게스트/배터/운영자/엔진/배당산출기/정산기)별 RLS 정책 정의
  - [~] 베팅·지갑은 `user_id = auth.uid()` 제한, 엔진 쓰기는 서비스롤 전용 — **57일차 로컬 마이그레이션 작성 완료, 원격 미적용**. `supabase/migrations/20260722070511_bet_tables.sql`이 §6.3.1 D그룹 잔여 5종(E-33~E-37 `bet_market`/`bet_selection`/`odds`/`bet`/`bet_leg`)의 테이블 생성 + RLS를 정의한다(E-38~E-40 지갑·프로필은 51~52일차 Task 037에서 완료). `bet`은 `(SELECT auth.uid()) = user_id` SELECT 전용, `bet_leg`는 복합키라 `user_id`가 없어 소유 `bet`을 EXISTS 서브쿼리로 간접 판정 — 수락 기준 "타 사용자 베팅 조회 차단"의 근거. `bet_market`/`bet_selection`/`odds`는 사용자 소유가 아닌 마켓·배당 데이터라 공개 SELECT + service_role 쓰기(A그룹 패턴). 전 테이블 쓰기는 **service_role 3분리 정책**(INSERT/UPDATE/DELETE — `FOR ALL` 대신 분리해 `multiple_permissive_policies` 경고 회피, profile/wallet 선례 준수). `wallet_transaction.ref_bet_id` FK도 52일차 예고대로 이월 추가. ⚠️ **원격 DDL 0건**(I-269 준수, 팀장이 `pg_class` 조회로 직접 확인) — 잔여 3단계는 `apply_migration` → `database.types.ts` 재생성 → `mapper.ts` 배팅 매퍼 추가이며 **착수 조건은 I-269 절차 정비**다. 적용 직전 마이그레이션 24~26행이 지시하는 `ref_bet_id` 기존 값 정합 확인 필수
  - [~] 감사 테이블(`audit_log`, `common_code_history`, 베팅 감사) append-only 강제 — **58일차 로컬 마이그레이션 작성 완료, 원격 미적용**. `supabase/migrations/20260722080000_audit_append_only.sql`이 **이중 방어**를 건다: ① RLS는 SELECT/INSERT 정책만 부여하고 **UPDATE/DELETE 정책을 아예 만들지 않으며**(PostgREST 경유 1차 차단선), ② `BEFORE UPDATE OR DELETE` 트리거 `reject_update_delete()`가 예외를 던져 **역할과 무관하게**(service_role의 direct SQL 포함) 거부한다 — service_role이 RLS를 우회하므로 정책만으로는 "강제"가 성립하지 않는다는 `docs/db/schema-design.md` §6.3.1(12일차 설계)을 그대로 구현한 것. **"베팅 감사"는 별도 테이블이 아니다** — §6.3.2가 명시하듯 `audit_log` 하나가 배팅 액터(`actor_type` ∈ `ODDS`/`SETTLEMENT`)까지 포괄하므로 대상은 2개 테이블. 채번은 직전 `20260722070511`과 충돌 없음. ⚠️ **원격 DDL 0건**(I-269 준수) — 따라서 수락 기준의 "UPDATE/DELETE 거부"는 **로컬 SQL 검증까지만**이고 원격 실측은 `apply_migration` 시점으로 이월(57일차 `bet_tables.sql`과 동일 패턴). ⚠️ **적용 전 I-283 선행 조치 필수** — SELECT를 service_role 전용으로 좁혀서, anon 키를 쓰는 `[lang]/admin/page.tsx:63`·`config/page.tsx:82`의 감사 조회가 **에러 없이 0행**이 된다
  - [~] 전 API 입력 스키마 검증(400 응답), 레이트 리밋(베팅 30/분, 공개 300/분) — **58일차 6팀 소유 API 검증분 완료, 레이트 리밋 미착수**. `src/app/api/admin/session/route.ts`의 POST body가 JSON `null`일 때 destructure에서 TypeError로 **500이 떨어지던 버그를 400으로 정정**(`typeof parsed !== "object" || parsed === null` 가드, 배열 body는 뒤의 타입 검사에서 `missing_credentials` 400). 회귀 테스트 2건 추가. `/api/health`는 GET/DELETE로 입력이 없어 해당 없음 — 6팀 소유 경로에는 이제 400 미반환 경로가 없다. **I-271 준수** — `proxy.ts` matcher 미변경, Route Handler가 `assertAdminSession()`을 직접 호출하는 방식 유지(**59일차가 기한**). **59일차 레이트 리밋 완료로 이 항목 종결** — 신규 `src/lib/data/supabase/rate-limit.ts`가 **슬라이딩 윈도우 로그** 방식(고정 윈도우의 경계 2배 버스트 결함 회피)으로 구현되며, 시간을 호출자가 주입해(`now: number`) 테스트 결정성을 확보하고, Deno(Edge Function)·Node(Route Handler) 양쪽에서 쓰이므로 런타임 전용 API·React·Supabase 클라이언트를 일절 쓰지 않는 순수 TS다. **베팅 30/분**은 `supabase/functions/wallet/index.ts`의 `reason === 'BET_PLACE'`에만 건다(WIN/VOID/TOPUP은 시스템 발신이라 대상 아님). **공개 300/분**은 실제 공개 라우트 2종(`api/live/matches`, `api/live/matches/[matchId]/events`)에 IP당 공유 버킷으로 걸고 초과 시 `429` + `Retry-After`를 반환한다 — ⚠️ **1차 보고는 이 버킷을 `/api/admin/session`에 걸고 "공개"로 분류했으나, 그 라우트는 어드민 인증 엔드포인트지 공개 API가 아니어서 팀장 재수정 지시로 정정**했다(실제 공개 라우트 적용 0건이었음). 어드민 세션 리미터는 브루트포스 방어용으로 **별도 인스턴스** 유지(키 공간 미공유). **런타임 실측** — `/api/live/matches` 305회 연속 요청에서 300건 허용 후 `429` 8건 관측으로 "초과 시 429"가 동작으로 확인됐다. 잔여: `/api/health`는 NFR-OB-004 "항상 200" 계약 + 모니터링 차단 방지로 **의도적 제외**(**I-285**, 팀장 수용), 어드민 세션 리미터 한도 300/분이 브루트포스 방어로는 과대(**I-288**), 인메모리라 멀티 인스턴스에서 카운트 분할(1차 릴리스 허용 근사치)
  - [x] 정책 테스트 스위트 — 액터별 허용·거부 케이스 전건 — **59일차 완료**. `supabase/policy-tests/`(`parse-policies.ts` + `rls-actor-matrix.test.ts`)가 **로컬 마이그레이션 SQL을 직접 파싱**해 검증한다(정책의 하드코딩 사본을 두지 않는 것이 핵심 — `src/lib/a11y/contrast.test.ts`가 `globals.css`를 파싱하는 것과 동일 패턴이라, 마이그레이션이 바뀌면 테스트가 따라 깨진다). 액터 4종(anon / authenticated 비소유 / authenticated 소유 / service_role) × 오퍼레이션 4종 × 테이블 11개 = **192건 전건 통과**. `profile.role`을 어느 정책도 참조하지 않는다는 **"운영자 DB 레벨 권한 상승 없음"** 불변식도 별도 단언한다. ⚠️ **스코프**: Group A 공개 읽기 34개와 `match_event`는 **로컬 SQL 자체가 없어**(**I-263**, 미문서 17건) 대상 밖이며, Task 038 본연의 "액터 세분화" 대상(B그룹 append-only 2종 + D그룹 8개 + 감사 1종 = 11개)은 전건 커버했다
- **수락 기준**: 정책 테스트 전건 통과. `get_advisors` 보안 경고 0건.
  - **59일차 판정** — 정책 테스트 192건 전건 통과. `get_advisors(security)`는 **문자 그대로의 0건은 미충족**이나(`security_definer_view: match_event_visible` · `auth_leaked_password_protection` 2건 관측), 팀장 확인 결과 `match_event_visible`은 **로컬 `supabase/` 전체에 정의가 0건이고 원격에만 존재**하는 **I-263 계열 사전 존재분**이며 59일차 원격 DDL은 0회다. 따라서 **"오늘 신규 유입 0건"으로 실질 충족 판정**하고, 실제 0건화는 **I-286**으로 이관해 I-263 해소(034b 전)와 한 패스로 처리한다.
