...
- **구현 사항**
  ...
  - [ ] 복합 컴포넌트: ~~`EventTimelineItem`~~, ~~`NewsItem`~~ (28일차 완료), ~~`PitchLineup`(7 포메이션)~~ (29일차 완료 — 7종 렌더 검증), ~~`BracketTree`~~ (**30일차 완료**), ~~`GrowthChart`~~, ~~`InjuryTimeline`~~ (**31일차 완료**), `TrophyCase`, **`MatchCard`**(27일차 SP-2 승격 — `density:"card"|"row"` 단일 통합, +0.6~0.7인일 증분)
...
- **수락 기준**: 22종 전부가 4상태를 지원하고, ko/en 전환 시 하드코딩 문자열 0건. (27일차 SP-2에서 `MatchCard` 승격으로 21→22종). **31일차 진행 19/22** (도메인 8/8 종결, 복합 5/7, **상태·유틸 6/6 종결**) — 잔여 3종은 `TrophyCase`·`MatchCard`(5팀) + 32~33일차 구간 — 4상태 판별 규약은 `state: DomainViewState|CompositeViewState<T>` 단일 prop + 리터럴 `loading|empty|error|ready`로 양 팀 통일(28일차 팀장 판정, I-156), 인터랙션 없는 컴포넌트는 서버 컴포넌트 + `t(locale, …)` 직접 호출.
