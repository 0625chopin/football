...
- **구현 사항**
  ...
  - [~] 클럽 상세 — 헤더(엠블럼·명성·팬), 스쿼드 테이블(부상·정지 배지), 감독/전술 카드, 시즌 지표, 재정 패널, 스폰서 3슬롯, 트로피, 최근/예정 경기 — **51일차 1/2 완료**(F1 헤더·F2 스쿼드 테이블+포지션 필터·상태 배지·OVR·F3 감독/전술). `teams/[teamId]/page.tsx` + `SquadTable.tsx`(신규). 진입 동선 확인 — `StandingsTable`이 이미 `/teams/[teamId]`로 링크(49일차 "링크 0건" 재발 아님). **F4~F8(시즌 지표·재정·스폰서·트로피·최근/예정 경기)은 53일차 2/2 잔여.** ⚠️ 상태 배지는 정상/부상/정지 3단(KNOCK 미분리 — 팀 단위 부상 배치 조회 계약 부재, I-255 계열). ⚠️ 창단·재임 시즌은 mock 생성값이 `currentSeason=1`과 충돌(음수/미래)해 **잠정 숨김**(`SHOW_SEASON_ORIGIN_FIELDS=false`, I-260 판정 후 되돌림)
