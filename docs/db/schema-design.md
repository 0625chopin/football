...
> 작성: 6팀 DB·인프라팀 / Task 009 / 9일차(2026-07-31) 착수, 10일차(2026-08-03) 갱신 중.
> **9일차**: 0~3장(테이블·컬럼·타입·제약) 작성 완료. **10일차(오늘)**: §6.1 관계 서술(R-01~13, 13개 전부) 추가 완료. 인덱스는 11일차, RLS·생명주기는 12일차에 이 파일에 이어서 채운다(§6 참조).
> **수락 기준(9일차, `docs/team-schedule/06-DB인프라팀.md`)**: E-01~E-47 **47개 엔티티 전량**의 테이블·컬럼·타입·제약 매핑. **10일차 수락 기준**: 관계 R-01~R-13(**13개 관계**) 반영 — `TeamSeason` 소속 관리, `Contract`/`Loan` 기반 선수-팀, `fixture.snapshot_id` NOT NULL 포함.
...
**10일차(오늘) 범위**는 위 9일차 산출물에 이어 다음을 추가한다(§6.1):

- 관계 서술(R-01~R-13, 13개) — FK 방향·카디널리티는 오늘 확정하고, 삭제 정책(ON DELETE) 상세는 제안값만 남기고 16일차(Task 032)로 이월

아직 다루지 **않는** 것(11~12일차 예정, §6):
...
|---|---|---|---|---|
| R-01 | World → League(3) → TeamSeason → Team | 팀의 리그 소속은 시즌마다 바뀌므로 `Team.league_id`를 두지 않고 `TeamSeason`으로 관리. 승강 히스토리·티어별 재적 시즌의 단일 근거 | `world.ts:117` `Team` 주석 "리그 소속은 승강으로 매 시즌 바뀌므로 `leagueId`를 두지 않는다. 소속의 단일 근거는 `TeamSeason`"; `world.ts:146` `TeamSeason` 주석 "승강 히스토리와 티어별 재적 시즌의 단일 근거" | §3.1 `team`(216행, `league_id` 컬럼 없음) + `team_season`(239행, `league_id` FK 보유) + 237행 각주 |
| R-07 | PointTransaction | `owner_type`으로 Team/Sponsor 원장을 통합 관리. `balance`는 원장의 파생 캐시이며 정합성 배치를 둔다 | `economy.ts:132` `PointTransaction` 주석 "`Team.balance`/`Sponsor.balance`는 이 원장의 파생 캐시다(회계 항등식, NFR-QA-005)" | §3.1 `team.balance`(233행), §3.7 `point_transaction`(650행) + 665행 회계 항등식 각주 |
| R-13 | Player → PlayerState | 1:1이지만 갱신 빈도가 매우 높아(라운드마다) `Player`에서 분리 — 쓰기 경합·캐시 무효화 범위 축소 | `person.ts:203` PlayerState 주석 "갱신 빈도가 매우 높아 분리했다(**R-13**, 쓰기 경합·캐시 무효화 범위 축소)" — TS 원문에 R-13 태그 명시 | §3.2 `player_state.player_id`(319행) + 323행 각주 |

#### 오늘 신규 서술 (10건)

| # | 관계 | 05문서 정의 | TS 재확인 근거 | 물리 표현(FK·카디널리티) | 삭제정책 |
|---|---|---|---|---|---|
| R-02 | Player ↔ Team | 직접 FK가 아니라 **활성 `Contract`**로 결정. 임대 중이면 `Loan`이 실제 출전 팀을 결정 — **출전 가능 팀 = `Loan.active ? loan_team : contract.team`** | `person.ts:213` `PlayerState.teamId` 주석 "R-02: 실제 출전 팀 결정은 Contract/Loan이 담당"; `person.ts:216` `onLoanTeamId` "임대 중 실제 출전 팀. 임대 아니면 null"; `economy.ts:49-58` `Contract`, `economy.ts:80-95` `Loan` | `player`(§3.2)엔 `team_id` 컬럼이 없다(의도적 설계). `player_state.team_id`(§3.2 324행, FK→`team.id`, null=FA)와 `player_state.on_loan_team_id`(325행, FK→`team.id`, null=임대 아님)가 "현재 유효 팀" 캐시이고, `contract(player_id, team_id, status)`(§3.3)·`loan(player_id, owner_team_id, loan_team_id, status)`(§3.3)가 원장. 카디널리티 Player 1:N Contract(시계열)·1:N Loan(시계열), 임의 시점 활성 건은 각 ≤1(부분 유니크 인덱스 후보) | `contract`/`loan` → `player`/`team` RESTRICT 제안(계약사 유실 방지) `[삭제정책 확정: 16일차 예정]` |
| R-03 | Fixture → MatchEvent (1:N) | 이벤트 로그가 **SSOT**. `PlayerMatchStat`·`TeamSeasonStat`은 이벤트에서 파생되며 언제든 재계산 가능해야 한다(FR-ST-005) | `match.ts:88` MatchEvent 주석 "스탯의 SSOT(**R-03**)"; `stat.ts:30` 헤더 "`TeamSeasonStat`에도 동일 원칙을 적용한다(**R-03**)"; `stat.ts:199` "중복 저장 시 **R-03** 회귀 추적이 두 곳으로 갈라짐" | `match_event.match_id`(§3.4 412행) FK→`fixture.id`, 1:N. `player_match_stat`/`player_season_stat`/`team_season_stat`은 별도 저장 테이블이지만 값은 `match_event` 집계로 언제든 재생성 가능해야 한다는 **불변식**이지 스키마 FK로 강제되는 관계는 아니다(운영/테스트 책임, NFR-QA 계열). 인덱스는 §3.4 424행에 이미 예고 | `fixture` 삭제 시 `match_event` CASCADE 제안 `[삭제정책 확정: 16일차 예정]` |
| R-04 | Fixture → SimConstantSnapshot (N:1) | **NOT NULL**. 결정론의 필수 축. 다수 Fixture가 동일 스냅샷을 공유한다(해시 중복 제거) | `match.ts:42,81-82` Fixture "`snapshotId`는 NOT NULL — 결정론 보장(FR-AD-014, DC-14)"; `config.ts:136` `SimConstantSnapshot.snapshotHash` "값 집합의 SHA-256(중복 제거 키). UNIQUE" | §3.4 `fixture.snapshot_id`(404행, 이미 NOT NULL 반영) FK→`sim_constant_snapshot.id`. N:1 — `snapshot_hash` UNIQUE가 동일 값 집합의 중복 스냅샷 생성을 막아 다수 Fixture의 공유·재사용을 유도 | `sim_constant_snapshot`은 참조 중 삭제 금지 — RESTRICT 제안 `[삭제정책 확정: 16일차 예정]` |
| R-05 | Standing (season × league × round × team) | 라운드별 순위 스냅샷. "N라운드 시점 순위" 조회와 시즌 마켓 배당 입력으로 사용 | `stat.ts:291-296` `Standing` 필드(`seasonId, leagueId, round, teamId, rank, ...`) — TS에 명시적 R-ID 태그는 없음, 구조로 확인 | §3.5 `standing`(551행) 복합 PK `(season_id, league_id, round, team_id)`, 최신 순위는 `round = MAX(round)`(566행 이미 명시). `season_id`/`league_id`/`team_id` 전부 N:1 FK | `season`/`league`/`team` 삭제 시 CASCADE 제안(스냅샷성 데이터) `[삭제정책 확정: 16일차 예정]` |
| R-06 | Sponsor ↔ Team (N:M via SponsorContract) | 팀당 활성 계약 ≤ 3 제약. 스폰서 부도 시 관련 계약 전건 `VOIDED` | `economy.ts:118-129` `SponsorContract(sponsorId, teamId, status)` — 팀당 ACTIVE≤3은 TS 레벨에서 강제하지 않음(§3.7 648행 "제약 예고(11일차 인덱스/16일차 CHECK)" 이미 명시) | `sponsor_contract.sponsor_id`(§3.7) FK→`sponsor.id`, `sponsor_contract.team_id` FK→`team.id` — N:M 조인 테이블. "부도 시 전건 VOIDED"는 애플리케이션/트리거 책임(3팀 소비), 스키마는 FK·상태 컬럼만 제공 | 물리 DELETE 대신 상태 전환(`VOIDED`)이 원칙 — `sponsor`→`sponsor_contract` RESTRICT 제안 `[삭제정책 확정: 16일차 예정]` |
| R-08 | BetMarket → BetSelection → Odds | 배당은 갱신되므로 이력 테이블로 두고 `is_current`로 현재값을 지정. `Bet.odds_snapshot`이 제출 시점 배당을 **동결**해 사후 배당 변경이 정산에 영향을 주지 않게 한다 | `betting.type-test.ts:32` "Odds.isCurrent로 최신값을 구분한다(이력 테이블 설계, **R-08**)" — TS 테스트 코드에 R-08 태그 명시 | §3.9 `bet_market.id`(701행) ← `bet_selection.market_id`(720행) ← `odds.selection_id`(731행), `odds.is_current` boolean(734행). `bet.odds_snapshot` jsonb(749행) + `bet_leg.odds_at_placement`(759행) 이중 동결. **전 테이블 2차 릴리스**(1차 마이그레이션 대상 아님, §3.9 헤더 명시) | 2차 릴리스 대상이라 오늘 확정 불필요 `[삭제정책 확정: 032 2차 착수 시점]` |
| R-09 | Award(인물) vs Trophy(클럽) | 조회 경로를 단순화하기 위해 분리. 선수 상세는 Award만, 클럽 상세는 Trophy만 조회 | `stat.ts:316-328` `Award`(`playerId`/`managerId`/`teamId` 배타적 nullable), `stat.ts:331-338` `Trophy`(`teamId`만, player/manager 필드 없음) — 구조로 확인, TS에 R-ID 태그 없음 | §3.8 `award`(669행)와 `trophy`(683행) — 독립 테이블, 서로 FK 참조 없음. Award의 "player/manager/team 중 하나만 non-null" 배타 규칙은 CHECK 후보 | `season` 삭제 시 CASCADE 제안 `[삭제정책 확정: 16일차 예정]` |
| R-10 | CommonCodeGroup → CommonCode → CommonCodeHistory | 그룹이 타입·범위·발효 정책을 정의하고, 코드가 값을 담고, 이력이 변경을 추적 | `config.ts:70` `CommonCode.groupCode` 주석 "FK → `CommonCodeGroup.groupCode`"; `config.ts:107-108` `CommonCodeHistory.commonCodeId` | §3.11 `common_code.group_code` FK→`common_code_group.group_code`(자연키, 817행), `common_code_history.common_code_id` FK→`common_code.id`(839행). 1:N:N 체인, history는 append-only(849행 이미 명시) | `common_code_group`은 구조상 삭제 없음 원칙(RESTRICT), `common_code_history`는 원본 삭제와 무관 보존(비정규화 `group_code`/`code` 컬럼이 근거, 840~841행) `[삭제정책 확정: 16일차 예정]` |
| R-11 | CommonCode → SimConstantSnapshot | 직접 FK가 아니라 **값 집합의 직렬화 결과**로 연결. 코드가 이후 변경되어도 스냅샷은 불변 | `config.ts:138` `SimConstantSnapshot.constants` "`{ "GROUP_CODE": { "CODE": value, ... }, ... }` 전체 값 집합"; `config.ts:136` `snapshotHash` "값 집합의 SHA-256" | §3.11 `sim_constant_snapshot`(851행)에 `common_code`로의 FK 컬럼은 **존재하지 않음**(의도적) — `constants` jsonb가 생성 시점 `common_code` 값 전체를 복사 저장하는 스냅샷이라 원본이 바뀌어도 과거 스냅샷은 불변 | 해당 없음(FK 자체가 없어 삭제 전파 대상 아님) |
| R-12 | PlayerAttribute vs PlayerAttributeHistory | 전자는 현재값(1:1), 후자는 시즌별 스냅샷(1:N). 성장 곡선 UI는 후자를 조회 | `person.ts:169` `PlayerAttribute` "Player와 1:1"; `person.ts:179-181` `PlayerAttributeHistory` "복합 키(`playerId`+`seasonNumber`). `PlayerAttributeValues`를 그대로 재사용해 `PlayerAttribute`와 필드 중복 선언을 만들지 않는다(C-6)" | §3.2 `player_attribute.player_id`(288행, PK/FK 1:1) vs `player_attribute_history(player_id, season_number)`(302행, 복합PK, 1:N) — 34속성 공유 블록은 이미 1회만 전개(300행 각주) | `player` 삭제 시 둘 다 CASCADE 제안(현재값·이력 모두 선수 종속) `[삭제정책 확정: 16일차 예정]` |
...
*이 문서는 11~12일차에 이어서 채운다(§6.2~§6.4). 10일차 작업 종료 시점 기준 최종본 — 9일차(0~3장) + 10일차(§6.1 관계 서술 R-01~R-13) 누적 반영.*
