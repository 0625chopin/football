...
```
NULL 통과 조건은 의도적이다 — 3팀 공통코드 카탈로그(**14일차 갱신: I-88 사용자 결정으로 36→37그룹** — 국가 목록·각국 비중 그룹 신규 추가, D-17 원문 준수. `common_code_group`/`common_code` 물리 구조는 그룹 수와 무관해 DDL 변경 없음)의 범위 값은 37일차 이후 그룹별 점진 채움 예정(§9.3)이라, 아직 범위가 안 정해진 코드는 이 CHECK를 무조건 통과해야 한다. JSON형 CHECK(`value_json NOT NULL WHEN value_type='JSON'`)는 05문서 570~574행 원안 유지, 오늘(13일차)은 컬럼 정의까지만.
...
**"과도기 NULL" 규약**: `min_value`/`max_value`가 `NULL`인 코드는 CHECK를 무조건 통과한다(하한/상한 없음, 또는 아직 값이 안 정해짐 — 두 의미를 구분하지 않고 동일하게 처리). 3팀 공통코드 카탈로그(37그룹 — **14일차 I-88 반영, 국가 목록·각국 비중 그룹 신규 추가로 36→37**, §10.2 참조)의 실제 범위 값은 **37일차 이후 그룹별 점진적으로 채워질 예정**(3팀 판단, `ODDS_PARAM`/`RATING_WEIGHT` 등 엔진 튜닝 그룹은 031b 밸런싱 루프(66~68일차)에서야 올바른 범위가 밝혀짐)이며, 16일차에 값을 미리 채우는 안은 억측이라 기각됐다. 채워지는 즉시 자동으로 CHECK가 걸리므로 별도 배포는 필요 없다.

**§8.2 H-08 대응표 갱신**: E-41 13→10, E-42 16→19(위 표 반영 완료). 1팀 재확인 시 이 문서 §3.11·§8.2를 기준으로 대조하면 된다.

---

## 10. 14일차(2026-08-07) 실행 인계 메모 — 공통코드+운영 7테이블 적용, I-88 그룹 수 정정

### 10.1 마이그레이션 적용

§3.11이 정의한 `common_code_group`(E-41)·`common_code`(E-42)·`common_code_history`(E-43)·`sim_constant_snapshot`(E-44)·`cron_run`(E-45)·`cron_gap`(E-46)·`audit_log`(E-47) 7개 테이블을 `apply_migration`으로 적용했다(원격 채번 버전 `20260720044145`, `supabase/migrations/20260720044145_common_code_ops_tables.sql`). §9.1이 예고한 대로 `fixture.snapshot_id`/`season.snapshot_id` → `sim_constant_snapshot(id)` FK도 같은 마이그레이션에서 이행했다(착수 직전 `fixture` 0행 재확인 완료). `list_tables` 39개 테이블(기존 32 + 신규 7) 확인, `get_advisors`에 신규 이슈 없음(security는 RLS 미적용류만/18일차 예정, performance는 unindexed_foreign_keys만/15일차 예정).

이번 마이그레이션은 `common_code_group`에 `min_value`/`max_value`/`json_schema`를 넣지 않고(§9.3 이동 반영) `common_code`에만 두었으며, 숫자형 CHECK는 16일차로 그대로 이월했다(§3.11 "제약 예고(16일차)" 유지). `common_code.updated_by`/`common_code_history.changed_by`는 `user_profile`(Task 037, 51~55일차) 테이블이 아직 없어 FK 없이 컬럼만 생성했다 — 037 완료 후 6팀이 ALTER TABLE로 FK를 마저 건다(인계 사항).

### 10.2 I-88 — 공통코드 그룹 36 → 37 (사용자 결정)

**결정**: 국가 목록·각국 비중을 공통코드 그룹으로 신규 추가한다(D-17 원문 준수). 3팀 `catalog.ts`/`fallback.ts`에 37번째 그룹이 추가된다. §3.11(863행)·§9.3(1470행)의 "36그룹" 서술을 37그룹으로 정정했다.

**물리 스키마 영향 없음**: `common_code_group`/`common_code`는 그룹 수에 의존하는 컬럼·CHECK가 없는 범용 구조라 DDL 변경이나 신규 마이그레이션이 필요 없다. 그룹이 36개든 37개든 같은 두 테이블에 행만 늘어난다. 시드 계획(3팀 Task 031a, 36~38일차, `supabase/seed/common-code.sql`)도 그룹 수와 무관하게 동일 절차로 37개를 적재하면 된다.

---

*이 문서는 12일차 작업으로 최종본이 되었다(§6.3 RLS·생명주기, §8 필드 대응표 추가). 누적 반영 — 9일차(§0~§3 개요·47엔티티 테이블) + 10일차(§6.1 관계 서술 R-01~R-13) + 11일차(§6.2 인덱스 설계, 14개 공식 인덱스 + 이월분 6건) + 12일차(§6.3 RLS 정책 초안·데이터 생명주기, §8 도메인 타입 필드 대응표 — H-08 인계) + 13일차(§9, 마이그레이션 실행 인계 메모 — snapshot_id FK 이월 금지선·파일 버전 명명 규칙·§9.3 `min_value`/`max_value`/`json_schema` E-41→E-42 이동) + **14일차(§10, 공통코드+운영 7테이블 apply_migration 실행·snapshot_id FK 이행, I-88 그룹 수 36→37 정정)**. Task 009(ROADMAP.md) 전 구현 사항 완료, 마이그레이션은 13일차(Task 032)부터 순차 진행 중.*
