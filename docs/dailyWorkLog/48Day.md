...
- ⚠️ 위 "코드 변경 0건"은 **Task 017 검증 범위(`matches/[matchId]`) 한정**이다 — 마감 커밋 직후 별건으로 `fixtures/page.tsx` 수정 1건이 들어갔다(아래 1-1절)
...
## 1-1. 마감 커밋 후 후속 수정 (`7bd11c6`)

스프린트 마감 커밋(`d06c213`) 직후 사용자가 **일정/결과 화면에서 경기 행을 눌러도 이동하지 않는다**고 제보해 별건으로 수정했다. 위 5팀 절의 "코드 변경 0건"은 Task 017 검증 대상(`matches/[matchId]`)에 대한 것이고, 이 건은 Task 016 산출물(`fixtures/page.tsx`)이라 스코프가 다르다.

- **원인**: 행 전체 링크(stretched link)가 `z-0`인데 각 셀 내용이 `relative z-10`이라 **내용이 링크를 완전히 덮고 있었다.** 셀 padding의 좁은 빈 틈을 정확히 눌렀을 때만 이동했고, 모바일에서는 그 틈이 사실상 없어 종료·라이브·예정 어느 행도 반응하지 않았다.
- **조치**: 링크를 `z-20`으로 올려 내용 위에 오게 했다. 내용의 `z-10`은 **빼지 않았다** — 그것이 짝수행 배경(`bg-muted/30`) 위로 내용을 올리는 역할도 겸하기 때문이다. 링크는 배경이 없어 위에 와도 텍스트를 가리지 않는다.
- **검증**: 390×844 Playwright 실브라우저 — 6개 행 × 4개 셀 **전부가 각자 행의 링크에 적중**(행 간 누수 0), 라이브/종료/예정 각 상태에서 실제 탭 → 경기 상세 이동 **5건** 확인. typecheck·lint 통과.
- **교훈**: stretched link 패턴은 **겹침 순서가 눈에 보이지 않아 시각 검증으로는 통과한다.** 마크업만 보면 정상이고 데스크톱에서는 우연히 눌리기도 한다 — 행 전체 링크를 쓰는 화면은 **셀 중앙 좌표를 실제로 탭해 적중을 확인**해야 한다. 이슈로 등재하지 않은 것은 원인·조치·검증이 이 항목에서 종결됐고 잔여 판정이 없기 때문이다.
