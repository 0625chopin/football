 ...
 *   (`blacklist.ts`).
 *
 * ## 왜 이름 풀(문자열)은 공통코드가 아니라 여기 정적 데이터로 남는가 (14일차 갱신)
 * 13일차 최초 버전은 "05문서 5.12.1절 표에 국적/이름 풀 그룹이 없어 12일차 동결
 * 카탈로그에 못 실었다"는 이유로 이 괴리 전체(국가 목록·비중·이름 풀)를 팀장 보고로
 * 넘겼다(I-88). **14일차 사용자 판단으로 결론이 갈렸다**: D-17 파급① 원문("국가 목록과
 * 각국 비중은 공통코드로 관리")을 그대로 지키기로 하고, `catalog.ts`에 37번째 그룹
 * `NATIONALITY_WEIGHT`를 신설했다(`SUPPORTED_NATIONALITY_CODES`와 동일한 코드 키 +
 * 국가별 배정 가중치, 실값은 031b 소관 — 현재는 `fallback.ts`에 빈 구조). **다만 이
 * 결정의 범위는 "국가 목록과 비중"이지 이름 풀 자체가 아니다** — D-17 파급①이 문자
 * 그대로 그렇게 말하고 있고, 이름(given)·성(family) 후보군 문자열 데이터를 공통코드
 * `JSON` 그룹으로 옮기는 것은 애초에 저장 성격이 다르다(관리자가 시즌마다 "튜닝"할
 * 값이 아니라 콘텐츠 데이터 — `blacklist.ts`와 같은 범주). 그래서 **이 파일(이름 풀
 * 문자열)은 계속 이 모듈이 직접 소유하는 정적 데이터로 남는다** — 범위를 넓히지
 * 않는다(팀장 지시, 14일차).
 *
 * ## 데이터 성격
