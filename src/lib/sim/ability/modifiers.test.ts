...
describe('개별 계수 함수 2종 — 골격 단계 클램프 통과 및 override 배선 (부상·홈, 미구현)', () => {
...
describe('conditionModifier — `M = 0.70 + 0.30×(C−1)/9` (18일차 실공식)', () => {
  ...
  it('base/range override(35일차 I-83 주입 패턴 반영)가 실제로 공식에 반영된다', () => {
    expect(conditionModifier({ condition: 1 }, { base: 0.5, range: 0.3 })).toBeCloseTo(0.5, 10);
    expect(conditionModifier({ condition: 10 }, { base: 0.5, range: 0.3 })).toBeCloseTo(0.8, 10);
  });
});
...
describe('fitnessModifier — `M = 0.75 + 0.25×(fitness/100)` (18일차 실공식)', () => {
  ...
  it('base/range override(35일차 I-83 주입 패턴 반영)가 실제로 공식에 반영된다', () => {
    expect(fitnessModifier({ fitness: 0 }, { base: 0.5, range: 0.4 })).toBeCloseTo(0.5, 10);
    expect(fitnessModifier({ fitness: 100 }, { base: 0.5, range: 0.4 })).toBeCloseTo(0.9, 10);
  });
});
