...
describe('enums ko/en 표시명 — 키 대칭(H-10 7그룹 70리터럴 + 31일차 injuryStatus 1그룹 2리터럴 + 33일차 trophyType 1그룹 4리터럴 + 35일차 newsFeedItemType 1그룹 10리터럴 = 11그룹 86리터럴)', () => {
  ...
  // 35일차: 5팀 홈 뉴스 요약 배지가 막혀 있던 `NewsFeedItemType`(E-26, 10종) 그룹을
  // 신설해 76 → 86으로 갱신(5팀 제보 — 카탈로그 공백 때문에 D-18 회피용으로 배지를
  // 의도적으로 생략해 두었던 케이스).
  const EXPECTED_LITERAL_COUNT = 86;

  it(`${groups.length}그룹 ${EXPECTED_LITERAL_COUNT}개 코드 리터럴 전부가 ko/en 양쪽에서 빈 문자열 없이 채워져 있다`, () => {
    let total = 0;
