...
`create-next-app`에서 출발한 Next.js 프로젝트입니다 (Next.js 16.2.10 / React 19.2.4 / App Router). **구현이 진행 중입니다 — 36일차 완료(2026-09-08 기준).** 일차별 상세는 `docs/dailyWorkLog/NDay.md`가 단일 소스이며, 이 절에는 **"새로 만들기 전에 알아야 할 현재 구조"만** 적습니다.

- **라우트 골격과 전역 레이아웃은 완성돼 있습니다.** `src/app/[lang]/`에 루트 레이아웃 1개 + 라우트 20개(각각 `page.tsx` + `loading`/`error`/`not-found` 3종 = 4상태 파일 60개)가 있습니다.
  - **4상태 파일 60개는 전부 얇은 래퍼입니다**(36일차) — 실제 마크업·로케일 처리는 `src/components/state/`의 `RouteLoading`/`RouteNotFound`/`RouteError` 3종에 있습니다. **개별 라우트 파일을 고치지 말고 그 껍데기를 고치세요**(60개가 주석 한 줄만 다른 동일 파일이라 개별 수정은 반드시 표류합니다 — 실제로 I-170 로케일 수정이 1곳에만 적용된 적이 있습니다). 라우트별로 다르게 보여야 할 것이 생길 때만 해당 파일에 직접 마크업을 둡니다.
  - **완전 미매치 URL의 404는 `src/app/global-not-found.tsx`가 뜹니다**(`[lang]/not-found.tsx`가 아닙니다 — 라우팅 자체를 우회하므로). 이 파일은 **자기 `<html>`을 직접 갖고 있어 레이아웃의 폰트 변수를 물려받지 못하므로** 폰트를 여기서도 선언합니다.
  ...
  - `SiteHeader`/`SideNav`/`MobileNav`/`SiteFooter`는 아직 **루트 레이아웃의 로컬 함수**입니다(23일차 이후 분리 예정이 36일차까지 미이행 — 4팀 소유, I-186에 묶임).
  - **사이드 내비 11개 중 3개(경기·플레이오프·팀)는 비활성 표시입니다**(36일차 I-186, 44일차 리그 해소, 49일차 마감 후 선수 해소) — 목록 화면(`page.tsx`)이 없어 누르면 404이기 때문입니다. `NAV_GROUPS`의 `pending: true` 플래그로 표시합니다. **49일차에 잔여 3종이 정식 배정됐습니다** — Task 046 `/teams`(5팀 60일차) · 047 `/matches`(5팀 61일차) · 048 `/playoffs`(4팀 50~51일차), `pending` 제거는 **4팀 62일차**(`layout.tsx`가 4팀 단독 소유라 화면 담당팀이 직접 못 지웁니다 — 인계 H-28).
    - ⚠️ **"Task 016~021이 채우면 플래그만 지우면 된다"는 I-186의 인계 문구는 틀렸습니다**(44일차 실사, I-223). 그 다섯 Task의 구현 사항은 **전부 상세 화면**(`[leagueId]`·`[matchId]` 등)이고 인덱스 화면은 한 줄도 없습니다 — "인덱스 화면"이라는 단어가 `ROADMAP.md`·`docs/team-schedule/**`에 **0건**입니다. 즉 **남은 3종은 담당 Task도 일차도 없는 상태**이며, 기다린다고 열리지 않습니다. 실제로 리그는 담당 Task 016이 42일차에 완료 처리되며 창구가 닫혔고, 44일차에 사용자 지시로 별건 신설(`leagues/page.tsx`)해 해소했습니다. **선수도 49일차 마감 후 같은 경로로 열렸습니다**(사용자 질의 → `players/page.tsx` 신설) — 두 번 다 "사용자가 물어봐서" 열렸다는 사실 자체가 이 항목이 방치되고 있다는 신호입니다.
    - **49일차에 이 상태가 해소됐습니다** — 잔여 3종이 Task 046~048로 배정됐고, 셋의 비용·가치가 달라 일괄 "필요/불필요" 판정은 기각됐습니다. **임의로 먼저 만들지 마세요**(담당 팀·일차가 있습니다). `/matches`는 **축소 스코프 확정**입니다 — 전역·날짜 범위 경기 조회 계약이 `DataSource`에 없어(I-255) 라이브 + 다음 킥오프 + 리그×라운드 폼까지만 만들며, **날짜 필터·통합 타임라인은 스코프 밖**입니다. **인덱스 3종은 와이어프레임이 없고, 앞으로도 만들지 않습니다** — 스코프 상한의 단일 소스는 `ROADMAP.md` Task 046~048의 수락 기준입니다.
    - 반대로 **상세 화면을 새로 만들 때는 "어디서 눌러 들어오는가"를 함께 확인하세요.** 49일차 선수 상세는 화면 자체는 정상이었으나 앱 내 링크가 0건이라 URL 직접 입력 외에는 도달할 수 없었습니다(49Day 1-1절).
...
- **컴포넌트 37종이 `src/components/`에 이미 있습니다**(`ui/` 12 · `domain/` 8 · `composite/` 8 · `state/` 9 + 순수 로직 파일). **새로 만들기 전에 반드시 목록부터 확인하세요.** 소유는 `ui`/`domain`/`state` = 4팀, `composite` = 5팀입니다.
- **⚠️ WSL 마운트(`/mnt/...`)에서 Turbopack이 실패합니다(I-62).** dev는 **`npm run dev`가 `WATCHPACK_POLLING=true next dev --webpack`으로 고정돼 있어 그대로 쓰면 됩니다**(번들러 고정은 13일차, 폴링은 36일차 I-185 조치 — 같은 마운트에서 inotify도 신뢰할 수 없어 **소스를 고쳐도 서버가 재컴파일하지 않습니다. 오류 없이 옛 화면이 그대로 떠서 코드 문제로 오진하기 쉽습니다**). Turbopack으로 직접 띄우면(`npx next dev`) 청크 쓰기가 EPERM으로 죽어 모든 페이지가 500이 되니 쓰지 마세요. **프로덕션 빌드는 번들러와 무관하게 실패**하므로(webpack 경로도 최종 `copyfile`에서 EPERM) 빌드 성공을 검증 수단으로 쓰지 말고 `npm run typecheck`(35일차부터 — 이전엔 `npx tsc --noEmit`, I-181로 대체됨) / `npm run lint` / `npm run test`로 판정하세요.
...
```bash
npm run dev     # 개발 서버 (http://localhost:3000) — WATCHPACK_POLLING 고정(I-185)
...
- **Prettier / pre-commit 훅 없습니다.** (Husky, lint-staged 미설치)
  - **타입체크는 `npx tsc --noEmit`을 직접 실행하지 말고 `npm run typecheck`을 쓰세요**(35일차, I-181). `tsconfig.json`의 `.next/dev/types/**` include는 Next.js 16이 dev/build 전환 시 tsconfig가 흔들리지 않도록 **의도적으로 관리하는 것**이라(지워도 다음 `next dev`/`next build`가 되살림) 손댈 수 없는데, 그 경로가 로컬에서 dev 서버가 띄워진 채로 있을 때 **계속 재생성되는 휘발성 산출물**이라 raw `npx tsc --noEmit`이 그 파일을 쓰는 도중에 읽어 무작위로 실패한다(WSL DrvFs torn write, I-62 계열). `npm run typecheck`(`scripts/typecheck.mjs`)은 오류가 전부 `.next/` 아래에서만 났을 때만 재시도하고, `src/**` 등 실제 오류가 하나라도 섞이면 재시도 없이 그대로 실패시킨다 — CI(dev 서버가 없어 이 경합 자체가 없음)에서도 안전하게 동일 명령을 쓸 수 있다. `npm run gate`(`scripts/gate.sh`)도 내부적으로 이 명령을 쓴다. **49일차 마감 후 추가(I-254)**: 이 스크립트는 tsc를 돌리기 전에 라우트 타입 산출물 2종(`.next/types/**`·`.next/dev/types/**`)이 공존하면 오래된 쪽을 지운다 — 둘 다 `tsconfig.json`의 include에 들어 있고(Next.js가 그렇게 관리한다) 각자 `AppRoutes`를 선언해서, `next build` 산출물이 남은 채 dev에서 **새 라우트를 추가하면 새 라우트가 `TS2344`로 오탐 실패**한다. 경고가 한 줄 찍히면 정상 동작이며, 지워진 쪽은 다음 `next dev`/`next build`가 되살린다.
...
- **비주얼 디렉션은 "Floodlit(야간 조명)"입니다** (36일차). 근거와 실측치는 `globals.css` 상단 주석이 단일 소스이며, 아래 3개 규칙만 지키면 됩니다.
  ...
  - **⚠️ 뷰포트 브레이크포인트 `sm`(375px)·`xs`(320px)는 레이아웃 전환에 쓰지 마세요**(36일차 I-184 확정). 이 프로젝트는 `sm`을 Tailwind 기본값(640px)이 아니라 **375px로 재정의**해 뒀고, `docs/wireframe/00-공통규약.md` §5가 `sm`을 **"320과 동일 취급"**으로 정의합니다 — 즉 열 수가 바뀌는 첫 지점은 **`md`(768px)**입니다. Tailwind 관용구를 그대로 쓰면 의도와 정반대로 동작합니다(`sm:grid-cols-2`는 통상 "좁은 화면에선 1열"을 뜻하는데 여기선 375px부터 2열이 됩니다 — 36일차에 홈 라이브 그리드가 실제로 밟아 팀명이 잘렸습니다).
    - ✅ `grid-cols-1 md:grid-cols-2 lg:grid-cols-3` / ❌ `grid-cols-1 sm:grid-cols-2`
    - `/sample`의 `@sm:`·`@lg:`는 **컨테이너 쿼리**라 별개 축입니다(뷰포트가 아니라 컨테이너 폭 기준) — 그대로 쓰세요.
- 폰트: `next/font/google`의 **Geist / Geist_Mono / Archivo / Gothic A1 4종**(**`src/app/[lang]/layout.tsx`**에서 CSS 변수로 주입 — 역할 분담은 위 스타일링 절과 그 파일 상단 주석 참조). 최상단 `src/app/layout.tsx`는 존재하지 않습니다(위 "10일차 결정" 참조).
