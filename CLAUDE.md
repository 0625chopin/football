...
- **라우트 골격과 전역 레이아웃은 완성돼 있습니다.** `src/app/[lang]/`에 루트 레이아웃 1개 + 라우트 20개(각각 `page.tsx` + `loading`/`error`/`not-found` 3종 = 4상태 파일 60개)가 있습니다.
  ...
  - **사이드 내비 11개 중 5개(리그·경기·플레이오프·팀·선수)는 비활성 표시입니다**(36일차 I-186) — 목록 화면(`page.tsx`)이 없어 누르면 404이기 때문입니다. `NAV_GROUPS`의 `pending: true` 플래그로 표시하며, **Task 016~021이 화면을 채우면 그 플래그만 지우세요.**
