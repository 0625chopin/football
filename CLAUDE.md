...
- **라우트 골격과 전역 레이아웃은 완성돼 있습니다.** `src/app/[lang]/`에 루트 레이아웃 1개 + 라우트 20개(각각 `page.tsx` + `loading`/`error`/`not-found` 3종 = 4상태 파일 60개)가 있습니다.
  ...
  - **사이드 내비 11개 중 3개(경기·플레이오프·팀)는 비활성 표시입니다**(36일차 I-186, 44일차 리그 해소, 49일차 마감 후 선수 해소) — 목록 화면(`page.tsx`)이 없어 누르면 404이기 때문입니다. `NAV_GROUPS`의 `pending: true` 플래그로 표시합니다.
    ...
    - 남은 3종은 **Task·일차 배정(schedule-planner) 또는 "인덱스 불필요" 결정**이 선행돼야 합니다. 임의로 만들지 말고 I-223의 판정을 먼저 확인하세요.
