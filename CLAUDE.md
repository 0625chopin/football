...
- **라우트 골격과 전역 레이아웃은 완성돼 있습니다.** `src/app/[lang]/`에 루트 레이아웃 1개 + 라우트 20개(각각 `page.tsx` + `loading`/`error`/`not-found` 3종 = 4상태 파일 60개)가 있습니다.
  ...
  - **사이드 내비 11개 중 4개(경기·플레이오프·팀·선수)는 비활성 표시입니다**(36일차 I-186, 44일차 리그 해소) — 목록 화면(`page.tsx`)이 없어 누르면 404이기 때문입니다. `NAV_GROUPS`의 `pending: true` 플래그로 표시합니다.
    - ⚠️ **"Task 016~021이 채우면 플래그만 지우면 된다"는 I-186의 인계 문구는 틀렸습니다**(44일차 실사, I-223). 그 다섯 Task의 구현 사항은 **전부 상세 화면**(`[leagueId]`·`[matchId]` 등)이고 인덱스 화면은 한 줄도 없습니다 — "인덱스 화면"이라는 단어가 `ROADMAP.md`·`docs/team-schedule/**`에 **0건**입니다. 즉 **남은 4종은 담당 Task도 일차도 없는 상태**이며, 기다린다고 열리지 않습니다. 실제로 리그는 담당 Task 016이 42일차에 완료 처리되며 창구가 닫혔고, 44일차에 사용자 지시로 별건 신설(`leagues/page.tsx`)해 해소했습니다.
    - 남은 4종은 **Task·일차 배정(schedule-planner) 또는 "인덱스 불필요" 결정**이 선행돼야 합니다. 임의로 만들지 말고 I-223의 판정을 먼저 확인하세요.
